...
Frameverse（仓库名 PicLayout）当前主线是 Rust 图像核心 + Electron/Vue 桌面应用，支持 Windows 和 macOS。Python Tkinter 版本仅作为旧版保留。
...
```bash
# macOS/Linux shell 包装入口
bash scripts/dev.sh

# Windows 与 macOS 通用入口
cd electron-app
npm ci
npm run dev
```
...
# 等价于：npm ci → npm run package
# Windows 输出 NSIS .exe；macOS 输出 DMG + ZIP
```
...
Windows sidecar 名为 `rust-core.exe`，macOS sidecar 名为 `rust-core`。
...
- sidecar 路径：打包后从 `process.resourcesPath` 读取；开发模式优先 `target/release/`，其次 `target/debug/`，并按操作系统选择扩展名

**类型同步注意**：NDJSON 协议类型统一定义在 `electron-app/shared/protocol.ts`，Electron API 定义在 `electron-app/shared/electron-api.ts`。不要在 main、preload 或 renderer 重复声明协议。`renderer` 侧在调用 IPC 前需 `JSON.parse(JSON.stringify(config))` 将 Vue reactive Proxy 转为纯对象，否则结构化克隆会失败。

### 代码质量

```bash
cd electron-app
npm run format       # Prettier 统一 TypeScript/Vue/CSS/YAML 风格
npm run check        # format check + typecheck + rustfmt + clippy + tests + build
```

Rust 代码使用 `rustfmt` 和 Clippy；Electron/Vue 代码使用 Prettier。构建与 Cargo 参数集中在 `package.json` 和 `scripts/cargo.mjs`，不要再复制平台分支脚本。

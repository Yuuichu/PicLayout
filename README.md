...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| 内容长边 | 40% | 单张图片在排版坐标中的长边比例，按最终长边换算 |
| 单图边框 | 1% | 单张图片四周留白比例，按最终长边换算 |
| 横向/纵向间隔 | 0% | 拼贴单元之间的间隔比例，按最终长边换算 |
| 最终外边距 | 自动 / 10% | 自动模式按列数在 10%→2% 间插值；自定义按最终长边换算 |
| 最终长边 | 10000 px | 输出 JPEG 的最终长边，Layout 中唯一的像素尺寸 |
| DPI | 300 | 输出分辨率 |
| 背景色 | white | 底色及边框颜色 |
